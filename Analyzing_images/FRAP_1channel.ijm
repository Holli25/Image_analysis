...
run("Stack to Hyperstack...", "order=xyczt(default) channels=1 slices=&zslices frames=&tframes display=Color");
...
while(yes != 1){
	...
	Dialog.addMessage("Are you satisfied with this registration?");
...
saveAs("Results", savepath_histograms+j+".csv");
}
run("Clear Results");
...
waitForUser("Measuring recovery", "Measure the recovery of the selected ROIs by using the TimeSeriesAnalyzer V3, use the 'Get Average' function. When you are finished, press 'OK'!");
IJ.renameResults("Time Trace(s)", "Results");
saveAs("Results", savepath_histograms+"Results.csv");
